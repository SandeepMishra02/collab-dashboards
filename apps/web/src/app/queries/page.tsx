...
export default function QueriesPage() {
  ...
  async function runSQL() {
    ...
    const result = await api("/queries/run", {
      method: "POST",
      json: { dataset_id: id, sql }, // <-- send JSON, not raw body
    });

    const r = result.rows || [];
    const c = result.columns || (r.length ? Object.keys(r[0]) : []);
    setRows(r);
    setColumns(c);

    if (!xKey && c.length) setXKey(c[0]);
    if (!yKey && c.length > 1) setYKey(c[1]);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 items-center">
        ...
        />
        <button className="btn" onClick={quick}>
          Quick Preview
        </button>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="space-y-3">
          ...
          <button className="btn btn-primary" onClick={runSQL}>
            Run
          </button>
